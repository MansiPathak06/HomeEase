...
export default function VendorDashboard() {
  ...
  const [broadcastLoading, setBroadcastLoading] = useState({});  // { [bookingId]: true/false }
  const socketRef2 = useRef(null);   // socket.io ref (wsRef is existing WS, this is new)
  ...
  // ADD this new useEffect AFTER the existing connectWS useEffect:
  useEffect(() => {
    const token = localStorage.getItem("token");
    ...
    socket.on("connect", () => {
      ...
      // Fetch any pending requests on connect
      fetchBroadcastRequests();
    });

    // New broadcast request popup
    socket.on("booking:new_request", (data) => {
      // Add to broadcast requests list (deduplicate)
      setBroadcastRequests(prev => {
        const exists = prev.find(r => r.id === data.bookingId);
        ...
        return [{
          ...
          advance_charge: data.advanceCharge,
        }, ...prev];
      });

      // Also trigger the existing Zomato-style alert
      // (reuse your existing triggerIncomingAlert with adapted data)
      triggerIncomingAlert({
        ...
        date: data.date,
        time: data.time,
        isBroadcast: true,   // new flag — used in accept handler below
      });
    });

    // Someone else accepted — remove from our list
    socket.on("booking:locked", ({ bookingId }) => {
      setBroadcastRequests(prev => prev.filter(r => r.id !== bookingId));
      showToast("ℹ️ A request was accepted by another vendor", "info");
    });

    // User accepted our suggested slot
    socket.on("slot:accepted", ({ bookingId, newDate, newTime }) => {
      showToast(`✅ Customer accepted your suggested slot: ${newDate} ${newTime}`);
      fetchAll(); // refresh bookings
      setBroadcastRequests(prev => prev.filter(r => r.id !== bookingId));
    });

    // User rejected our suggested slot
    socket.on("slot:rejected", ({ bookingId }) => {
      showToast("⚠️ Customer rejected your suggested time slot.", "error");
      setBroadcastRequests(prev => prev.filter(r => r.id !== bookingId));
    });
  ...
  const fetchAll = async () => {
    ...
    try {
      const [bkRes, statsRes, profileRes, revRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/vendor/bookings`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/vendor/stats`, { headers: { Authorization: `Bearer ${token}` } }),

        fetch(`${process.env.NEXT_PUBLIC_API_URL}/vendor/profile`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/vendor/reviews`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
      if (bkRes.ok) {
  ...
  const updateStatus = async (bookingId, status, extra = {}) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/vendor/bookings/${bookingId}`, {
        method: "PUT",
  ...
  // Fetch pending broadcast requests from server (REST fallback)
  const fetchBroadcastRequests = async () => {
    const token = localStorage.getItem("token");
    if (!token) return;
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/vendor/broadcast-requests`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (data.success) setBroadcastRequests(data.requests || []);
    } catch (e) { console.error("fetchBroadcastRequests:", e); }
  };

  // Accept a broadcast request
  const handleBroadcastAccept = async (bookingId) => {
    setBroadcastLoading(prev => ({ ...prev, [bookingId]: true }));
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/vendor/broadcast-requests/${bookingId}/accept`,
      ...
      if (data.success) {
        ...
        setBroadcastRequests(prev => prev.filter(r => r.id !== bookingId));
        dismissAlert();
        fetchAll(); // refresh main bookings list
      } else if (data.alreadyTaken) {
        showToast("⚠️ Too late — another vendor accepted first.", "error");
        setBroadcastRequests(prev => prev.filter(r => r.id !== bookingId));
      } else {
        showToast(data.message || "Failed to accept", "error");
      }
    } catch { showToast("Network error", "error"); }
    finally { setBroadcastLoading(prev => ({ ...prev, [bookingId]: false })); }
  };

  // Decline a broadcast request
  const handleBroadcastDecline = async (bookingId) => {
    setBroadcastLoading(prev => ({ ...prev, [bookingId]: true }));
    try {
      const token = localStorage.getItem("token");
      await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/vendor/broadcast-requests/${bookingId}/decline`,
        { method: "POST", headers: { Authorization: `Bearer ${token}` } }
      );
      setBroadcastRequests(prev => prev.filter(r => r.id !== bookingId));
  ...
  // Suggest alternate slot modal states (add alongside existing modal states)
  const [showSuggestSlot, setShowSuggestSlot] = useState(false);
  const [suggestSlotBk, setSuggestSlotBk] = useState(null);
  ...
  const handleSuggestSlot = async () => {
    ...
    setBroadcastLoading(prev => ({ ...prev, [suggestSlotBk.id]: true }));
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/vendor/broadcast-requests/${suggestSlotBk.id}/suggest-slot`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ suggested_date: suggestedDate, suggested_time: suggestedTime }),
        }
      );
      const data = await res.json();
      if (data.success) {
        showToast("📅 Slot suggestion sent to customer!");
        setShowSuggestSlot(false);
        setBroadcastRequests(prev => prev.filter(r => r.id !== suggestSlotBk.id));
      } else if (data.alreadyTaken) {
        showToast("⚠️ Request already taken.", "error");
        setBroadcastRequests(prev => prev.filter(r => r.id !== suggestSlotBk.id));
        setShowSuggestSlot(false);
      } else {
        showToast(data.message || "Failed", "error");
    ...
    finally { setBroadcastLoading(prev => ({ ...prev, [suggestSlotBk?.id]: false })); }
  };
  ...
  const handleSendQuote = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/vendor/bookings/${selectedBk.id}/quote`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  //   try {
  //     const token = localStorage.getItem("token");
  //     const res = await fetch( `${process.env.NEXT_PUBLIC_API_URL}/vendor/bookings/${selectedBk.id}/final-payment`, {
  //       method: "POST",
  //       headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  const handleFinalPayment = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/vendor/bookings/${selectedBk.id}/final-payment`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ final_payment_method: paymentMethod }),
      ...
      if (data.success) {
        ...
        showToast(
          `✅ Payment recorded! You earn: ₹${Number(s.vendorPayout).toLocaleString("en-IN")} ` +
          `(Admin commission: ₹${Number(s.adminEarning).toLocaleString("en-IN")})`
        );
  ...
  const BroadcastRequestCard = ({ bk }) => {
    const isLoading = broadcastLoading[bk.id];
    const expiresAt = bk.expires_at ? new Date(bk.expires_at) : null;
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{
              fontSize: 9, fontWeight: 800, background: "rgba(0,0,0,0.3)",
        ...
        <div style={{ padding: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
            {[
              ...
              ["📅 Date", bk.date],
              ["🕐 Time", bk.time],
              ["💰 Budget", bk.budget ? `₹${Number(bk.budget).toLocaleString("en-IN")}` : "—"],
              ["📍 Address", bk.address ? bk.address.slice(0, 40) + (bk.address.length > 40 ? "…" : "") : "—", "span2"],
            ].map(([k, v, span]) => (
              <div key={k} style={{
                background: "#111", borderRadius: 6, padding: "8px 10px",
                ...(span === "span2" ? { gridColumn: "1/-1" } : {}),
              }}>
          ...
          <div style={{
            ...
            background: "#2D1800", borderRadius: 6, padding: "6px 10px",
            marginBottom: 12,
          }}>
            ℹ️ ₹99 visitation charge applies (existing flow)
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              disabled={isLoading}
              onClick={() => handleBroadcastAccept(bk.id)}
              style={{
            ...
            <button
              ...
              onClick={() => {
                setSuggestSlotBk(bk);
                setSuggestedDate(bk.date || "");
                setSuggestedTime("");
                setShowSuggestSlot(true);
              }}
              style={{
                flex: 1, padding: "9px 0", borderRadius: 8,
            ...
            <button
              disabled={isLoading}
              onClick={() => handleBroadcastDecline(bk.id)}
              style={{
                padding: "9px 12px", borderRadius: 8,
                background: "transparent", border: `1px solid ${T.darkBorder2}`,
                color: T.gray600, fontSize: 12, cursor: "pointer",
                fontFamily: "inherit", opacity: isLoading ? 0.5 : 1,
              }}>
              ✗
            </button>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 20 }}>
          {[
            ...
            { label: "Rating", value: parseFloat(stats.averageRating || 0).toFixed(1) + "★", color: T.crimson, icon: Star },
  ...
  return (
    <>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "20px 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10, marginBottom: 18 }}>
            {[
              ...
              { icon: Star, color: T.crimson, label: "Rating", value: parseFloat(stats.averageRating || 0).toFixed(1) + "★" },
                  ...
                  <div style={{ marginBottom: 24 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                      <div style={{
                        ...
                        backgroundColor: "#C0392B", animation: "pulse 1s infinite",
                      }} />
                      <p style={{ fontSize: 13, fontWeight: 800, color: "#F87171", margin: 0 }}>
                        {broadcastRequests.length} New Request{broadcastRequests.length > 1 ? "s" : ""} — Respond quickly!
                      </p>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column" }}>
                      {broadcastRequests.map(bk => (
                        <BroadcastRequestCard key={bk.id} bk={bk} />
                      ))}
                    </div>
                  </div>
                )}
      ...
      {showPaymentModal && selectedBk && (() => {
        // final_amount = total agreed price (set when user accepted quote)
        // advance_amount = ₹99 booking fee (already collected by platform)
        // remaining = what vendor collects on-site from customer
        // total = advance + remaining = full agreed price
        const totalAmt = Number(selectedBk.final_amount || selectedBk.quote_amount || selectedBk.amount || 0);
        const advanceAmt = Number(selectedBk.advance_amount || 99);
        const remaining = Math.max(0, totalAmt - advanceAmt);
        const commPct = Number(selectedBk.commission_pct || 10);
        const adminEarn = Math.round(totalAmt * commPct / 100);
        const vendorEarn = totalAmt - adminEarn;
        ...
        return (
          <Overlay onClose={() => setShowPaymentModal(false)}>
            <ModalCard>
              ...
              {/* ── Payment Breakdown ── */}
              <div style={{ backgroundColor: "#0D1F0D", border: "1px solid #1A3A1A", borderRadius: 10, padding: "14px 16px", marginBottom: 16 }}>
                <p style={{ fontSize: 10, color: "#4ADE80", fontWeight: 800, textTransform: "uppercase", marginBottom: 10, letterSpacing: "0.08em" }}>
                  Payment Breakdown
                </p>
                {[
                  // ✅ FIXED: advance is PART of total, not subtracted — customer already paid it
                  ["Advance (booking fee, already paid)", `₹${advanceAmt.toLocaleString("en-IN")}`, "#FCD34D"],
                  ["Remaining (you collect on-site)", `₹${remaining.toLocaleString("en-IN")}`, "#F97316"],
                  ["───────────────────────────────────", "", "#2a2a2a"],
                  ["Total agreed price", `₹${totalAmt.toLocaleString("en-IN")}`, "#fff"],
                  [`Platform commission (${commPct}%)`, `₹${adminEarn.toLocaleString("en-IN")}`, "#F1948A"],
                  ["Your earnings", `₹${vendorEarn.toLocaleString("en-IN")}`, "#4ADE80"],
                ].map(([label, val, color]) => {
                  if (label.startsWith("───")) return (
                    <div key={label} style={{ borderTop: "1px solid #1A3A1A", margin: "6px 0" }} />
                  );
                  return (
              ...
              {/* ── How did customer pay the REMAINING amount ── */}
              <p style={{ fontSize: 12, color: T.gray600, marginBottom: 10 }}>
                How did the customer pay the remaining{" "}
                <strong style={{ color: "#fff" }}>₹{remaining.toLocaleString("en-IN")}</strong>?
              </p>
                ...
                {[
                  ...
                  <div key={key} onClick={() => setPaymentMethod(key)}
                    style={{
                      ...
                      border: `2px solid ${paymentMethod === key ? "#059669" : "#2E2E2E"}`,
                      backgroundColor: paymentMethod === key ? "#052E16" : "#1A1A1A",
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setShowPaymentModal(false)}
                  style={{
                    flex: 1, padding: 12, borderRadius: 8, border: `1px solid ${T.darkBorder2}`,
                    background: "transparent", color: T.gray600, cursor: "pointer", fontWeight: 700, fontSize: 13
                  }}>
                  Cancel
                </button>
                <button onClick={handleFinalPayment} disabled={!paymentMethod || actionLoading}
                  style={{
                    flex: 2, padding: 12, borderRadius: 8, border: "none", background: "#059669",
                    color: "#fff", cursor: "pointer", fontWeight: 800, fontSize: 13,
                    opacity: (!paymentMethod || actionLoading) ? 0.5 : 1
                  }}>
                  {actionLoading ? "Recording..." : `✅ Confirm — ₹${remaining.toLocaleString("en-IN")} collected`}
                </button>
              </div>
            </ModalCard>
          </Overlay>
      ...
      {showSuggestSlot && suggestSlotBk && (
  <Overlay onClose={() => setShowSuggestSlot(false)}>
    <ModalCard>
      <ModalHead
        icon={RefreshCw}
        iconBg="#0D2340"
        iconColor="#93C5FD"
        title="Suggest Different Slot"
        sub={`${suggestSlotBk.service_name} — ${suggestSlotBk.customer_name}`}
        onClose={() => setShowSuggestSlot(false)}
      />
 
      <div style={{
        background: "#1C1400", border: "1px solid #3A2800",
        borderRadius: 8, padding: "8px 12px", marginBottom: 14,
        fontSize: 12, color: "#FDE68A",
      }}>
        Customer requested: {suggestSlotBk.date} at {suggestSlotBk.time}
      </div>
 
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        <div>
          <FormLabel required>Your available date</FormLabel>
          <input
            type="date"
            value={suggestedDate}
            min={today}
        ...
        <div>
          <FormLabel required>Your available time</FormLabel>
          <input
            type="time"
            value={suggestedTime}
      ...
      <div style={{ fontSize: 11, color: T.gray500, marginBottom: 16 }}>
        Customer will be notified and can accept or reject your suggestion.
        If they reject, the request may go to other vendors.
      </div>
 
      <div style={{ display: "flex", gap: 8 }}>
        <button
          onClick={() => setShowSuggestSlot(false)}
          style={{
            flex: 1, padding: 12, borderRadius: 8,
            border: `1px solid ${T.darkBorder2}`,
            background: "transparent", color: T.gray600,
        ...
        <button
          ...
          disabled={!suggestedDate || !suggestedTime || broadcastLoading[suggestSlotBk?.id]}
          style={{
            flex: 1, padding: 12, borderRadius: 8,
            border: "none", background: "#2563EB",
            color: "#fff", cursor: "pointer", fontWeight: 700, fontSize: 13,
            opacity: (!suggestedDate || !suggestedTime) ? 0.5 : 1,
          }}>
          Send Suggestion
        </button>
      </div>
